...
/**
 * Tour details.
 * GET /api/tours/details.php?id=&currency=RUB
 */
...
$path = '/tours/' . rawurlencode($tourId) . '?' . np_query_encode(['currency' => $currency]);
...
if (!$meta['ok']) {
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Tourvisor tour details failed',
...
if (!is_array($payload)) {
    user_sync_json_error('Invalid tour payload', 502);
}
...
try {
    ...
    if (!$img && !empty($payload['picture']) && is_string($payload['picture'])) {
        $img = $payload['picture'];
    }
    if ($hid > 0 && $img) {
    ...
    error_log('[tours/details] image cache optional: ' . $e->getMessage());
}

user_sync_json_ok($payload);
