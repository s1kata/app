# Документация TravelHub

| Документ | Описание |
|----------|----------|
| [**PRODUCTION.md**](./PRODUCTION.md) | **Главный документ для продакшена**: переменные окружения, EAS, оплата, CRM, OTA, чеклист |
| [RELEASE_CHECKLIST.md](./RELEASE_CHECKLIST.md) | Ручные проверки на устройстве перед/после релиза |
| [PAYMENT_STORES.md](./PAYMENT_STORES.md) | Оплата вне приложения и соответствие правилам App Store / Google Play |
| [PREVIEW_BUILD.md](./PREVIEW_BUILD.md) | Сборка preview для внутреннего теста (Android + iOS) |
| [SOTA_CRM_INTEGRATION.md](./SOTA_CRM_INTEGRATION.md) | Настройка интеграции с U-ON (SOTA) |
| [SOTA_CRM_API.md](./SOTA_CRM_API.md) | Краткий справочник API U-ON |
| [TERMS_OF_SERVICE.md](./TERMS_OF_SERVICE.md) | Условия использования |
| [PRIVACY_POLICY.md](./PRIVACY_POLICY.md) | Политика конфиденциальности |
| [../src/config/releaseUiFlags.ts](../src/config/releaseUiFlags.ts) | Флаги скрытого UI (отели / next-patch) |
| [../server/README.md](../server/README.md) | API оплаты для размещения на сайте |

**Поддержка пользователей:** hello@travelhub63.ru, тел. +7 (495) 660-36-66 (`tel:+74956603666`) — см. также экран «Помощь и поддержка» в приложении.

## Актуальные ограничения релизной ветки

- Персональные рекомендации на фронте временно скрыты (перенесены в следующий патч).
- Изменения, связанные с улучшенными уведомлениями и расширением hotel-флоу, держатся отдельно в next patch.

Корень проекта: [README.md](../README.md).
